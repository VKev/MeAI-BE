...
[ApiController]
[Route("api/User/instagram")]
public sealed class InstagramController : ApiController
{
    ...
    [HttpGet("authorize")]
    [Authorize]
    [ProducesResponseType(typeof(Result<InstagramOAuthInitiationResponse>), StatusCodes.Status200OK)]
        ...
        CancellationToken cancellationToken)
    {
        ...
        var result = await _mediator.Send(
            new InitiateInstagramOAuthCommand(userId, scopes),
            cancellationToken);
    ...
    {
        var result = await _mediator.Send(
            new CompleteInstagramOAuthCommand(code ?? "", state ?? "", error, errorDescription),
            cancellationToken);
