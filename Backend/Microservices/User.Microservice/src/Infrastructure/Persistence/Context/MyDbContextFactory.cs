    ...
    public MyDbContext CreateDbContext(string[] args)
    {
        ...
        if (string.IsNullOrWhiteSpace(connectionString))
        {
            ...
            var database = config["Database:Name"] ??
                           config["DATABASE__NAME"] ?? config["DATABASE_NAME"] ?? "userservice_db";
            var username = config["Database:Username"] ??
                           config["DATABASE__USERNAME"] ?? config["DATABASE_USERNAME"] ?? "postgres";
            var password = config["Database:Password"] ??
                           config["DATABASE__PASSWORD"] ?? config["DATABASE_PASSWORD"] ?? "password";
            var sslMode = config["Database:SslMode"] ??
                          config["DATABASE__SSLMODE"] ?? config["DATABASE_SSLMODE"] ?? "Prefer";
            connectionString =
