...
public sealed record GetMyReportPreviewQuery(
    Guid ReportId,
...
public sealed class GetMyReportPreviewQueryHandler : IQueryHandler<GetMyReportPreviewQuery, AdminReportPreviewResponse>
{
    private readonly IUnitOfWork _unitOfWork;
    private readonly IUserResourceService _userResourceService;

    public GetMyReportPreviewQueryHandler(IUnitOfWork unitOfWork, IUserResourceService userResourceService)
    {
        _unitOfWork = unitOfWork;
        _userResourceService = userResourceService;
    }

    public async Task<Result<AdminReportPreviewResponse>> Handle(GetMyReportPreviewQuery request, CancellationToken cancellationToken)
    {
        ...
        if (report.ReporterId != request.RequestingUserId)
        {
            return Result.Failure<AdminReportPreviewResponse>(FeedErrors.Forbidden);
        }
    ...
    private async Task<Result<AdminReportPreviewResponse>> BuildCommentPreviewAsync(
        Report report,
        GetMyReportPreviewQuery request,
        CancellationToken cancellationToken)
    {
