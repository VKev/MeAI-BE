"""NanoVectorDB-backed storages.

NanoVectorDBStorage: untouched (used for entities/relations/chunks in text RAG
when no Qdrant adapter is wired in).

NanoVectorDBVideoSegmentStorage: kept upstream-compatible signature but rewritten
to use our OpenRouter-based encode_video_segments (Gemini Embedding 2 multimodal)
instead of ImageBind. The `embedder` arg is gone — feature.encode_video_segments
ignores it anyway.
"""
import asyncio
import os
...
class NanoVectorDBVideoSegmentStorage(BaseVectorStorage):
    ...
    async def upsert(self, video_name, segment_index2name, video_output_format):
        ...
        for _batch in tqdm(batches, desc=f"Encoding Video Segments {video_name}"):
            batch_embeddings = encode_video_segments(_batch).numpy()
            all_vectors.append(batch_embeddings)
        embeddings = np.concatenate(all_vectors, axis=0) if all_vectors \
            else np.zeros((0, self.global_config["video_embedding_dim"]), dtype=np.float32)
        for i, d in enumerate(list_data):
            d["__vector__"] = embeddings[i]
        results = self._client.upsert(datas=list_data)
        return results
