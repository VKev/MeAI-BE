...
except Exception:  # pragma: no cover — only used in load_caption_model(debug=False)
...
class VideoRAG:
    ...
    def load_caption_model(self, debug=False):
        # caption model — when wired to OpenRouter we always run with debug=True
        # so the local MiniCPM weights are never loaded.
        if not debug:
            if AutoModel is None or AutoTokenizer is None:
                raise RuntimeError(
                    "transformers not installed; call load_caption_model(debug=True) "
                    "to skip the local caption model when using OpenRouter."
                )
