...
- `FeedController` mặc định yêu cầu đăng nhập.
- Các endpoint đọc public gồm:
...
- User hiện tại được backend lấy từ `ClaimTypes.NameIdentifier` nếu có.
- Với endpoint yêu cầu auth, nếu không resolve được user id hợp lệ, backend trả `401` với `MessageResponse("Unauthorized")`.
...
|---|---|---|
| GET | `/api/Feed/profiles/{username}` | Lấy public profile theo username, cho phép anonymous |
| GET | `/api/Feed/profiles/{username}/posts` | Lấy timeline post public theo username, cho phép anonymous và có cursor pagination |
| POST | `/api/Feed/posts` | Tạo bài viết mới |
| GET | `/api/Feed/posts/feed` | Lấy feed của user hiện tại, có cursor pagination |
| GET | `/api/Feed/posts/{id}` | Lấy chi tiết một bài viết, cho phép anonymous |
...
| GET | `/api/Feed/posts/{id}/comments` | Lấy **root comments** của bài viết, có cursor pagination, cho phép anonymous |
| GET | `/api/Feed/comments/{id}/replies` | Lấy replies của một comment, có cursor pagination, cho phép anonymous |
| POST | `/api/Feed/comments/{id}/reply` | Tạo reply cho một comment |
| DELETE | `/api/Feed/comments/{id}` | Xóa comment/thread của chính mình |
...
| GET | `/api/Feed/admin/reports` | Lấy danh sách report cho admin, hỗ trợ filter `status` và `targetType` |
| PATCH | `/api/Feed/admin/reports/{id}` | Admin review report, đổi trạng thái và action moderation |
...
- `createdAt`
- `updatedAt`
- `isLikedByCurrentUser`: `boolean | null`, null khi request anonymous
- `canDelete`: `boolean | null`, null khi request anonymous
...
Lấy public profile theo username để render profile header, kể cả khi người xem chưa đăng nhập.

### Gợi ý frontend

- endpoint này an toàn cho guest/anonymous page
- có thể dùng để hydrate profile header trước khi load timeline post
- nếu backend trả `Feed.User.NotFound` thì nên render trạng thái profile không tồn tại
- counts `followersCount` và `followingCount` đã được tổng hợp sẵn từ Feed service
...
Lấy timeline post public của một username cụ thể.

### Gợi ý frontend

- phù hợp cho tab posts trong màn profile public
- endpoint cho phép anonymous; khi user chưa đăng nhập thì `isLikedByCurrentUser` và `canDelete` sẽ là `null`
- mỗi `PostResponse` đã có sẵn `username` và `avatarUrl`, nên frontend không cần join thêm author profile để render post card trong luồng đọc
- media trong post được resolve bằng public resource lookup, nên guest vẫn có thể render ảnh/video nếu resource hợp lệ
- khi user đã đăng nhập, backend sẽ fill viewer flags để frontend render nút like/delete chính xác hơn
- tiếp tục dùng cùng cursor strategy như home feed
...
- response create hiện vẫn theo shape `PostResponse`, nhưng `username` và `avatarUrl` có thể đang là fallback (`"unknown"` / `null`) ngay sau khi tạo; nếu cần render tức thời, frontend nên ưu tiên profile hiện có của current user
...
- mỗi post item đã có `username` và `avatarUrl`, nên có thể render author header trực tiếp từ response
...
- response detail đã có `username`, `avatarUrl` và `isLikedByCurrentUser`, nên frontend không cần gọi thêm API riêng để render author info hoặc biết trạng thái like hiện tại
...
## 10) GET `/api/Feed/posts/{id}/comments`
...
## 11) GET `/api/Feed/comments/{id}/replies`
...
## 12) POST `/api/Feed/comments/{id}/reply`
...
## 13) POST `/api/Feed/follow/{userId}` và DELETE `/api/Feed/follow/{userId}`
...
## 14) GET `/api/Feed/followers/{userId}`
...
## 15) GET `/api/Feed/following/{userId}`
...
## 17) GET `/api/Feed/admin/reports`

### Query params

- `status`: optional, hỗ trợ `Pending`, `InReview`, `Resolved`, `Dismissed`
- `targetType`: optional, hỗ trợ `Post` hoặc `Comment`

### Gợi ý frontend

- chỉ dùng ở admin area
- frontend nên guard route theo role trước khi render màn admin
- có thể map filter UI trực tiếp sang `status` và `targetType`
- nếu backend trả `401/403` thì chuyển hướng khỏi admin page

---

## 18) PATCH `/api/Feed/admin/reports/{id}`
...
- dùng cho moderation drawer hoặc admin detail panel
- nếu chọn action `DeleteTargetPost` thì nên chỉ enable khi report target là post và status đích là `Resolved`
- sau review thành công nên cập nhật row hiện tại thay vì refetch toàn bộ nếu dataset lớn
- response trả lại moderation metadata mới nhất gồm `reviewedByAdminId`, `reviewedAt`, `resolutionNote`, `actionType`
...
- mỗi post card có author avatar/name, preview media, hashtags, comments count
...
- fetch profile header qua `GET /api/Feed/profiles/{username}`
- fetch timeline post qua `GET /api/Feed/profiles/{username}/posts`
- nút follow/unfollow
...
- `Feed.Report.Status`: hiển thị “Trạng thái moderation không hợp lệ”
- `Feed.Report.Action`: hiển thị “Action moderation không hợp lệ”
- `Feed.Report.StatusTransition`: hiển thị “Không thể chuyển report sang trạng thái này”
- `Feed.Report.ActionTarget`: hiển thị “Action hiện tại không hỗ trợ cho loại target này”
- `Feed.Report.ActionStatus`: hiển thị “Action này yêu cầu report ở trạng thái Resolved”
