...
Tài liệu này đã được cập nhật theo trạng thái backend hiện tại trong `User.Microservice`.

### API đã triển khai

- [x] `GET /api/User/admin/storage/settings/free-tier`
- [x] `PUT /api/User/admin/storage/settings/free-tier`
- [x] `GET /api/User/admin/storage/settings/system`
- [x] `PUT /api/User/admin/storage/settings/system`
- [x] `GET /api/User/admin/storage/plans`
- [x] `GET /api/User/admin/storage/plans/{subscriptionId}`
- [x] `PATCH /api/User/admin/storage/plans/{subscriptionId}`
- [x] `GET /api/User/admin/subscriptions`
- [x] `GET /api/User/admin/subscriptions/{subscriptionId}`
- [x] `POST /api/User/admin/subscriptions`
- [x] `PUT /api/User/admin/subscriptions/{subscriptionId}`
- [x] `PATCH /api/User/admin/subscriptions/{subscriptionId}`
- [x] `GET /api/User/admin/storage/usage`
- [x] `GET /api/User/admin/storage/usage/users/{userId}`
- [x] `GET /api/User/admin/storage/resources`
- [x] `POST /api/User/admin/storage/cleanup/run`
- [x] `POST /api/User/admin/storage/reconcile`
- [x] `GET /api/User/resources/storage-usage`
- [x] `POST /api/User/resources`
- [x] `PUT /api/User/resources/{id}`

### Đã có trong code nhưng contract thực tế khác phần “đề xuất”

- `GET /api/User/admin/storage/usage`: response hiện dùng `subscriptionId` và `subscriptionName`, không dùng `planId` và `planName`.
- `GET /api/User/admin/storage/resources`: response hiện chưa trả `expiresAt`.
- `GET /api/User/resources/storage-usage`: response hiện trả thêm `userId`, `subscriptionId`, `subscriptionName`, `isOverQuota`; FE nên ignore field thừa nếu không cần.

### Chưa triển khai đầy đủ theo thiết kế dài hạn

- [ ] `user_storage_usages` table và cơ chế `reserved_bytes` thật sự chống race condition ở DB level.
- [ ] `expires_at` trên `resources`.
- [ ] Cleanup theo retention/reference graph hoàn chỉnh.
- [ ] Response quota error giàu metadata như ví dụ `errors.quotaBytes`, `errors.usedBytes`, `errors.requestedBytes`, `errors.availableBytes`.
...
- `resources` đã có `size_bytes`, `storage_provider`, `storage_bucket`, `storage_region`, `storage_namespace`, `storage_key`, `original_file_name`, `last_verified_at`, `deleted_from_storage_at`.
- Chưa thấy `etag`, `checksum_sha256`, `storage_class`, `expires_at`.
- Chưa có bảng `user_storage_usages`; usage hiện đang được tính từ bảng `resources`.
...
```http
GET /api/User/admin/storage/resources?userId={guid?}&workspaceId={guid?}&resourceType=image|video&includeDeleted=true&namespace={text?}&cursorCreatedAt={iso?}&cursorId={guid?}&limit=50&includePresignedUrl=true
```

Response thực tế hiện tại trả các field:
...
Để frontend hoạt động với backend hiện tại, nên bám đúng contract thực tế dưới đây thay vì chỉ bám ví dụ “đề xuất”.
...
- `quotaBytes === null`: hiển thị unlimited.
- `quotaBytes === 0`: hiển thị uploads disabled.
- `isOverQuota === true`: disable action upload/create mới, nhưng vẫn cho list/preview/delete resource cũ.
- Với admin usage, dùng `subscriptionId` và `subscriptionName` đúng như response hiện tại.
- Với admin resources, không giả định có `expiresAt` cho đến khi backend bổ sung field này.
...
### Trạng thái hiện tại so với acceptance criteria

- [x] Admin can view used storage per user and compare it with that user's current plan quota.
- [x] Admin can create/update/patch subscription plans with `storage_quota_bytes`, `max_upload_file_bytes`, and `retention_days_after_delete`.
- [x] Users with a storage-limited subscription receive exactly that plan quota at runtime.
- [x] Upload is rejected before S3 write when known file size exceeds available quota.
- [x] User over quota after a plan downgrade cannot upload/create any new resource, but can still list, preview, download, and delete existing resources.
- [ ] Concurrent uploads cannot exceed quota through race conditions.
- [x] S3 keys are always namespaced per dev/environment.
- [x] Resetting local PostgreSQL does not affect other developers' S3 objects because namespaces are isolated.
- [x] Cleanup can run dry-run and real mode, and never deletes outside the configured namespace.
- [x] Subscription plan API accepts and returns storage quota fields without breaking existing FE response shape.
- [x] Admin has an API to list over-quota users by subscription plan.
- [x] Existing resource APIs keep their current `Result<T>` success envelope and `ProblemDetails` failure shape.
