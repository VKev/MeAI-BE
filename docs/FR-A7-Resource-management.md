...
- `dryRun=true`: scan S3 prefix, báo object nào không có trong DB.
- `dryRun=false` + `mode=mark-missing`: đánh dấu DB resources không còn object.
- `dryRun=false` + `mode=import-orphans`: chỉ dùng local/dev, import object vào một owner fallback nếu key parse được user id; nếu không parse được thì giữ orphan report.

Không nên tự động import orphan vào production vì S3 object không đủ thông tin business để khôi phục user/subscription chính xác nếu key không chuẩn.
...
11. Add cleanup/reconciliation service with dry-run first.
